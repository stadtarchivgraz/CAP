...
class Starg_Admin_Notification {
	...
	public static function starg_check_dependencies() {
		...
		try {
			...
			$notification_message[] = sprintf( __( '%1$s In order to create thumbnails from PDF files, you might need to change the security policy of ImageMagick. Check for policy.xml in your ImageMagick installation folder. Change the line %2$s to %3$s', 'sip' ), $warning_sign, $image_magick_policy_line_1, $image_magick_policy_line_2 );
		}
		...
		if ( carbon_get_theme_option( 'sip_clamav' ) ) {
			if ( ! function_exists( 'socket_create' ) ) {
				...
				if (! $clamav_rdy) {
					...
					$notification_message[] = sprintf(esc_attr__('%1$s There is an error in your configuration for ClamAV. We cannot connect to the ClamAV server. %2$s.', 'sip'), $error_sign, $clamav_settings_link);
				}
			}
		}
