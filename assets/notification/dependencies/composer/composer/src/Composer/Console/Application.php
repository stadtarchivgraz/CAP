...
use BracketSpace\Notification\Dependencies\Composer\Command;
...
use BracketSpace\Notification\Dependencies\Composer\Util\ErrorHandler;
use BracketSpace\Notification\Dependencies\Composer\Util\HttpDownloader;
use BracketSpace\Notification\Dependencies\Composer\EventDispatcher\ScriptExecutionException;
use BracketSpace\Notification\Dependencies\Composer\Exception\NoSslException;
...
';
    ...
    public function __construct(string $name = 'BracketSpace\Notification\Dependencies\Composer', string $version = '')
    {
            ...
            if (is_file($file) && Filesystem::isReadable($file) && is_array($composer = json_decode(file_get_contents($file), true))) {
                if (isset($composer['scripts']) && is_array($composer['scripts'])) {
                    foreach ($composer['scripts'] as $script => $dummy) {
                        if (!defined('BracketSpace\Notification\Dependencies\Composer\Script\ScriptEvents::'.str_replace('-', '_', strtoupper($script)))) {
                            if ($this->has($script)) {
     ...
     * @return \BracketSpace\Notification\Dependencies\Symfony\Component\Console\Command\Command[]
     */
    ...
    private function getPluginCommands(): array
    {
        ...
        if (null !== $composer) {
            ...
            foreach ($pm->getPluginCapabilities('BracketSpace\Notification\Dependencies\Composer\Plugin\Capability\CommandProvider', ['composer' => $composer, 'io' => $this->io]) as $capability) {
