...
require_once( STARG_SIP_PLUGIN_BASE_DIR . 'inc/form-validation/filter-archival-query.class.php' );
...
<form id="sip-filter" action="" method="get" class="container mb-5">
	<div class="columns is-multiline">
		<?php
		$archive     = esc_attr( $filter_input['filter-archive'] );
		$tag         = esc_attr( $filter_input['filter-tag'] );
		$purpose     = esc_attr( $filter_input['filter-purpose'] );
		...
		<?php
		// only show the archive selection dropdown, if we have multiple archives set.
		if ( ! $single_archive_id ) :
			...
			<?php endif; ?>
		<?php endif; ?>
		<div class="column <?php echo ( $single_archive_id ) ? 'is-full' : 'is-6-tablet'; ?>">
			<div class="field">
		...
		<div class="column is-one-quarter-desktop is-6-tablet">
			<div class="field">
				<label for="filter-post-status"><?php esc_html_e('Status', 'sip'); ?></label>
				<div class="control">
					<div class="select is-fullwidth">
						<select name="filter-post-status" id="filter-post-status" class="postform">
							<option <?php selected( $post_status, 0 ); ?> value="0"><?php esc_html_e('Show all', 'sip'); ?></option>
							<option <?php selected( $post_status, 'draft' ); ?> value="draft"><?php esc_html_e('Draft', 'sip'); ?></option>
							<option <?php selected( $post_status, 'pending' ); ?> value="pending"><?php esc_html_e('Pending', 'sip'); ?></option>
							<option <?php selected( $post_status, 'publish' ); ?> value="publish"><?php esc_html_e('Accepted', 'sip'); ?></option>
						</select>
					</div>
				</div>
			</div>
